...
IMPLEMENT_DYNAMIC(CDLG_SX6, CDialog)

CDLG_SX6::CDLG_SX6(CWnd* pParent /*=nullptr*/)
	: CDialog(IDD_DIALOG_SX6, pParent)
{
...
void CDLG_SX6::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
}


BEGIN_MESSAGE_MAP(CDLG_SX6, CDialog)
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONDBLCLK()
	ON_WM_MOUSEMOVE()
	ON_WM_RBUTTONDOWN()
...
bool CDLG_SX6::DrawLine(CPoint point) {
	//设置绘图模式
...
bool CDLG_SX6::bIsPtInRegion(int ix, int iy) {
	int pointNum = 0;
...
bool CDLG_SX6::FloodFill(int x, int y, COLORREF OldColor, COLORREF NewColor) {
	CDC* pDC = this->GetDC();
...
void CDLG_SX6::OnLButtonDown(UINT nFlags, CPoint point)
{
	//如果多边形已经绘制完成，则擦除
	if (bFlag) {
		Invalidate(TRUE);
...
void CDLG_SX6::OnMouseMove(UINT nFlags, CPoint point)
{
...
void CDLG_SX6::OnLButtonDblClk(UINT nFlags, CPoint point)
{
...
void CDLG_SX6::OnRButtonDown(UINT nFlags, CPoint point)
{
	if (bFlag) {
		if (bIsPtInRegion(point.x, point.y)) FloodFill(point.x, point.y, RGB(128, 128, 128), RGB(55, 0, 0));
		else MessageBox(_T("种子点不在区域内"));
	}
