...
void readFile(double &f, int &num, double *&x_left, double *&y_left,
  ...
  sscanf(tmp.data(), "%lf%lf%lf%lf", &x_left[0], &y_left[0], &x_right[0],
         &y_right[0]);
  for (int i = 1; i < num; i++) {
    getline(data, tmp);
    sscanf(tmp.data(), "%lf%lf%lf%lf", &x_left[i], &y_left[i], &x_right[i],
           &y_right[i]);
  }
  for (int i = 0; i < num; i++) {
    x_left[i] /= 1000.0;
...
int main() {
  ...
  double r_angles[3] = {0}, r_coord[3] = {5.185 / 1000.0, 0, 0}, u = 0,
         v = 0;   //相对定向元素
  double n1, n2;  //投影系数
  ...
  while (true) {
    ...
    v += X[4][0];
    //判断改正数是否满足限制
    if (fabs(X[0][0]) < limit && fabs(X[1][0]) < limit &&
        fabs(X[2][0]) < limit && fabs(X[3][0]) < limit && fabs(X[4][0]) < limit)
      break;
  }
  fprintf(stdout, "%.6lf %.6lf %.6lf %.6lf %.6lf \n", r_angles[0], r_angles[1],
          r_angles[2], u, v);
  delete[] x_left;
  delete[] x_right;
  delete[] y_left;
