...
double Matrix::value() {
  ...
  for (int i = 0; i < rows; i++) {
    if (i % 2 == 0)
      value += minor(i, 0).value() * p[i][0];
    else
      value += minor(i, 0).value() * p[i][0] * (-1);
  }
  return value;
}
...
Matrix Matrix::adjt() {
  ...
  for (int i = 0; i < cols; i++) {
    for (int j = 0; j < rows; j++) {
      if ((i + j) % 2 == 0)
        tmp.p[i][j] = minor(j, i).value();
      else
        tmp.p[i][j] = minor(j, i).value() * (-1);
    }
  }
  return tmp;
}
...
double*& Matrix::operator[](int index) { return p[index]; }
...
void readFile(double& f, double& x0, double& y0, int& m, int& num, double**& pp,
              double**& gp) {
  ...
  sscanf(tmp.data(), "%lf%lf%lf%lf%lf", &pp[0][0], &pp[0][1], &gp[0][0],
         &gp[0][1], &gp[0][2]);
  for (int i = 1; i < num; i++) {
    getline(data, tmp);
    sscanf(tmp.data(), "%lf%lf%lf%lf%lf", &pp[i][0], &pp[i][1], &gp[i][0],
           &gp[i][1], &gp[i][2]);
  }
  for (int i = 0; i < num; i++) {
    pp[i][0] /= 1000.0;
...
void init(double (&eo_coords)[3], double& f, int& m, int& num, double**& gp,
          double**& ap) {
  ap = new double*[num];
  for (int i = 0; i < num; i++) {
    ap[i] = new double[2];
...
void cal_R(Matrix& R, double (&eo_angles)[3]) {
  R[0][0] = cos(eo_angles[0]) * cos(eo_angles[2]) -
            sin(eo_angles[0]) * sin(eo_angles[1]) * sin(eo_angles[2]);
  R[0][1] = -cos(eo_angles[0]) * sin(eo_angles[2]) -
            sin(eo_angles[0]) * sin(eo_angles[1]) * cos(eo_angles[2]);
  ...
  R[2][0] = sin(eo_angles[0]) * cos(eo_angles[2]) +
            cos(eo_angles[0]) * sin(eo_angles[1]) * sin(eo_angles[2]);
  R[2][1] = -sin(eo_angles[0]) * sin(eo_angles[2]) +
            cos(eo_angles[0]) * sin(eo_angles[1]) * cos(eo_angles[2]);
  R[2][2] = cos(eo_angles[0]) * cos(eo_angles[1]);
}

void apxm(double (&eo_coords)[3], int& num, double& f, double**& gp,
          double**& ap, Matrix& R) {
  for (int i = 0; i < num; i++) {
    ap[i][0] = -f *
               (R[0][0] * (gp[i][0] - eo_coords[0]) +
                R[1][0] * (gp[i][1] - eo_coords[1]) +
                R[2][0] * (gp[i][2] - eo_coords[2])) /
               (R[0][2] * (gp[i][0] - eo_coords[0]) +
                R[1][2] * (gp[i][1] - eo_coords[1]) +
                R[2][2] * (gp[i][2] - eo_coords[2]));
    ap[i][1] = -f *
               (R[0][1] * (gp[i][0] - eo_coords[0]) +
                R[1][1] * (gp[i][1] - eo_coords[1]) +
                R[2][1] * (gp[i][2] - eo_coords[2])) /
               (R[0][2] * (gp[i][0] - eo_coords[0]) +
                R[1][2] * (gp[i][1] - eo_coords[1]) +
                R[2][2] * (gp[i][2] - eo_coords[2]));
  }
...
void cal_A(Matrix& A, Matrix& R, double**& pp, double**& gp, double& f,
           double (&eo_angles)[3], double (&eo_coords)[3], int& num) {
  for (int i = 0; i < num; i++) {
    double Z = R[0][2] * (gp[i][0] - eo_coords[0]) +
               R[1][2] * (gp[i][1] - eo_coords[1]) +
               R[2][2] * (gp[i][2] - eo_coords[2]);
    ...
    A[i * 2][3] =
        pp[i][1] * sin(eo_angles[1]) -
        (pp[i][0] *
             (pp[i][0] * cos(eo_angles[2]) - pp[i][1] * sin(eo_angles[2])) / f +
         f * cos(eo_angles[2])) *
            cos(eo_angles[1]);  // a14
    A[i * 2][4] = -f * sin(eo_angles[2]) - pp[i][0] *
                                               (pp[i][0] * sin(eo_angles[2]) +
                                                pp[i][1] * cos(eo_angles[2])) /
                                               f;              // a15
    A[i * 2][5] = pp[i][1];                                    // a16
    ...
    A[i * 2 + 1][3] =
        -pp[i][0] * sin(eo_angles[1]) -
        (pp[i][1] *
             (pp[i][0] * cos(eo_angles[2]) - pp[i][1] * sin(eo_angles[2])) / f -
         f * sin(eo_angles[2])) *
            cos(eo_angles[1]);  // a24
    A[i * 2 + 1][4] =
        -f * cos(eo_angles[2]) - (pp[i][1] * (pp[i][0] * sin(eo_angles[2]) +
                                              pp[i][1] * cos(eo_angles[2]))) /
                                     f;  // a25
...
int main() {
  ...
  do {
    ...
    if (fabs(X[3][0]) < 0.01 && fabs(X[4][0]) < 0.01 && fabs(X[5][0]) < 0.01)
      break;
  } while (true);
  fprintf(stdout, "%.2lf %.2lf %.2lf %.6lf %.6lf %.6lf\n", eo_coords[0],
          eo_coords[1], eo_coords[2], eo_angles[0], eo_angles[1], eo_angles[2]);
  return 0;
}