...
void readFile(int &num, double *&pp_x, double *&pp_y, double *&pp_z,
              double *&gp_x, double *&gp_y, double *&gp_z) {
  ...
  sscanf(tmp.data(), "%lf%lf%lf%lf%lf%lf", &pp_x[0], &pp_y[0], &pp_z[0],
         &gp_x[0], &gp_y[0], &gp_z[0]);
  for (int i = 1; i < num; i++) {
    getline(data, tmp);
    sscanf(tmp.data(), "%lf%lf%lf%lf%lf%lf", &pp_x[i], &pp_y[i], &pp_z[i],
           &gp_x[i], &gp_y[i], &gp_z[i]);
  }
}
...
int main() {
  ...
  while (true) {
    ...
    for (int i = 0; i < num; i++) {
      ...
      L[3 * i + 0][0] =
          gp_x[i] -
          l * (R[0][0] * pp_x[i] + R[0][1] * pp_y[i] + R[0][2] * pp_z[i]) - x;
      L[3 * i + 1][0] =
          gp_y[i] -
          l * (R[1][0] * pp_x[i] + R[1][1] * pp_y[i] + R[1][2] * pp_z[i]) - y;
      L[3 * i + 2][0] =
          gp_z[i] -
          l * (R[2][0] * pp_x[i] + R[2][1] * pp_y[i] + R[2][2] * pp_z[i]) - z;
      //计算A
      A[3 * i + 0][0] = 1;
      A[3 * i + 0][3] = pp_x[i];
      A[3 * i + 0][4] = -pp_z[i];
